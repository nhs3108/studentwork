...
function ws_bubbles(c, m, o) {
    ...
    var i = f("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).addClass("ws_effect ws_bubbles").appendTo(o.parent());
    if (!j) {
    ...
    var t = [["#bbbbbb", 0.5, 0.5], ["#b3b3b3", 0.2, 0.2], ["#b6b6b6", 0.5, 0.2], ["#b9b9b9", 0.8, 0.2], ["#cccccc", 0.2, 0.8], ["#c3c3c3", 0.5, 0.8], ["#c6c6c6", 0.8, 0.8]];
    var d = [[[0.5, 0.5, 0.7, 0.15], [0.5, 0.5, 0.6, 0.3], [0.5, 0.5, 0.5, 0.45], [0.5, 0.5, 0.4, 0.6], [0.5, 0.5, 0.3, 0.75], [0.5, 0.5, 0.2, 0.9], [0.5, 0.5, 0.1, 1]], [[0.5, 0.5, 0.7, 1], [0.5, 0.5, 0.6, 0.9], [0.5, 0.5, 0.5, 0.75], [0.5, 0.5, 0.4, 0.6], [0.5, 0.5, 0.3, 0.45], [0.5, 0.5, 0.2, 0.3], [0.5, 0.5, 0.1, 0.15]]];
    var n = [[[0.5, 0.5, 0, 1], [0.5, 0.5, 0, 0.9], [0.5, 0.5, 0, 0.75], [0.5, 0.5, 0, 0.6], [0.5, 0.5, 0, 0.45], [0.5, 0.5, 0, 0.3], [0.5, 0.5, 0, 0.15]], [[0.5, 0.5, 0, 0.15], [0.5, 0.5, 0, 0.3], [0.5, 0.5, 0, 0.45], [0.5, 0.5, 0, 0.6], [0.5, 0.5, 0, 0.75], [0.5, 0.5, 0, 0.9], [0.5, 0.5, 0, 1]], [[0.5, 7.5, 0.7, 0.75], [0.5, 7.5, 0.6, 0.15], [0.5, 7.5, 0.5, 1], [0.5, 7.5, 0.4, 0.3], [0.5, 7.5, 0.3, 0.45], [0.5, 7.5, 0.2, 0.6], [0.5, 7.5, 0.1, 0.9]], [[0.5, 7.5, 0.7, 1], [0.5, 7.5, 0.6, 0.9], [0.5, 7.5, 0.5, 0.75], [0.5, 7.5, 0.4, 0.6], [0.5, 7.5, 0.3, 0.45], [0.5, 7.5, 0.2, 0.3], [0.5, 7.5, 0.1, 0.15]]];
    function e(v) {
        if (Object.prototype.toString.call(v) === "[object Array]") {
            ...
            return/linear|swing/g.test(h) ? e(v) : h
        }
    }
    function a(w, v, y) {
    ...
    function r(C, B, w, A, u) {
        C.clearRect(0, 0, l, q);
        for (var v = 0, z = w.length; v < z; v++) {
            var h = Math.max(0, Math.min(1, B - w[v][3] * (1 - B)));
            if (u && k[u]) {
                h = k[u](1, h, 0, 1, 1, 1)
            }
            var x = l;
            ...
            var y = w[v][2] * h * x;
            if (A) {
                y = (w[v][2] + (A[v][2] - w[v][2]) * h) * x
            }
            y = Math.max(0, y);
            C.beginPath();
            C.arc((w[v][0] + ((A ? A[v][0] : 0.5) - w[v][0]) * h) * l, (w[v][1] + ((A ? A[v][1] : 0.5) - w[v][1]) * h) * q, y, 0, 2 * Math.PI, false);
            C.fillStyle = t[v][0];
            C.fill()
        }
    }
    this.go = function (C, x) {
        if (j) {
            o.find(".ws_list").css("transform", "translate3d(0,0,0)").stop(true).animate({left: (C ? -C + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))}, c.duration, "easeInOutExpo", function () {
                g.trigger("effectEnd")
            })
        } else {
            l = o.width();
            q = o.height();
            b.attr({width: l, height: q});
            var A = m.get(x);
            for (var y = 0, B = t.length; y < B; y++) {
                var v = Math.abs(t[y][1]), h = Math.abs(t[y][2]);
                t[y][0] = s(A, {x: v * l, y: h * q, w: 2, h: 2}) || t[y][0]
            }
            var u = e(d);
            var w = e(n);
            var z = e(k);
            a(c.duration / 2, function (D) {
                r(p, D, u, 0, z)
            }, function () {
                o.find(".ws_list").css({left: (C ? -C + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))});
                z = e(k);
                a(c.duration / 2, function (D) {
                    r(p, 1 - D, w, u, z)
                }, function () {
...
// http://javascript-source.com
//***********************************************
jQuery.extend(jQuery.easing, {easeOutBackCubic: function (e, f, a, j, i, g) {
        var h = (f /= i) * f;
        return a + j * (-1.5 * h * f * h + 2 * h * h + 4 * h * f + -9 * h + 5.5 * f)
    }});
function ws_dribbles(q, l, a) {
    var e = jQuery;
    var k = e(this);
    var o = q.noCanvas || !document.createElement("canvas").getContext;
    var n = q.width, f = q.height;
    var j = e("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).addClass("ws_effect ws_dribbles").appendTo(a.parent());
    if (!o) {
        var c = e("<canvas>").css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%"}).appendTo(j);
        var p = c.get(0).getContext("2d")
    }
    var m = [["#bbbbbb", 0.1, 0.3, 0.18], ["#b3b3b3", 0.9, 0.8, 0.15], ["#b6b6b6", 0.68, 0.4, 0.2], ["#b9b9b9", 0.25, 0.4, 0.15], ["#cccccc", 0.11, 0.7, 0.15], ["#c3c3c3", 0.18, 0.1, 0.15], ["#c6c6c6", 0.4, 0.2, 0.15], ["#c9c9c9", 0.55, -0.04, 0.18], ["#d3d3d3", 0, 0.95, 0.13], ["#d6d6d6", 0.5, 0.8, 0.22], ["#d9d9d9", 0.9, 0.1, 0.18], ["#eeeeee", 0.3, 0.9, 0.18], ["#e3e3e3", 0.93, 0.5, 0.14], ["#e6e6e6", 0.7, 0.9, 0.15]];
    ...
    function b(z, x, u, r) {
        ...
        for (var s = 0, w = u.length; s < w; s++) {
            var t = 2 - u[s][3];
            var y = u[s][2] * (1 - x);
            var h = Math.max(0, Math.min(1, x * t - y));
            if (r && e.easing[r]) {
                h = e.easing[r](1, h, 0, 1, 1, 1)
            }
            var v = n;
            if (n / f <= 1.8 && n / f > 0.7) {
                v *= 2
            ...
            z.beginPath();
            z.arc((u[s][0] + (m[s][1] - u[s][0]) * h) * n, (u[s][1] + (m[s][2] - u[s][1]) * h) * f, m[s][3] * h * v, 0, 2 * Math.PI, false);
            z.fillStyle = m[s][0];
            z.fill()
        }
    }
    this.go = function (y, t) {
        if (o) {
            a.find(".ws_list").css("transform", "translate3d(0,0,0)").stop(true).animate({left: (y ? -y + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))}, q.duration, "easeInOutExpo", function () {
                k.trigger("effectEnd")
            })
        } else {
            n = a.width();
            ...
            var z = d[v];
            var s = d[Math.floor(Math.random() * (d.length))];
            c.attr({width: n, height: f});
            var w = l.get(v == 0 ? t : y);
            for (var u = 0, x = m.length; u < x; u++) {
                var r = Math.abs(m[u][1]), h = Math.abs(m[u][2]);
                m[u][0] = i(w, {x: r * n, y: h * f, w: 2, h: 2}) || m[u][0]
            }
            g(q.duration / 2, function (A) {
                b(p, A, z, "easeOutBackCubic")
            }, function () {
                a.find(".ws_list").css({left: (y ? -y + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))});
                g(q.duration / 2, function (A) {
                    b(p, 1 - A, s, "easeOutBackCubic")
                }, function () {
                    p.clearRect(0, 0, n, f);
                    k.trigger("effectEnd")
                })
    ...
    function i(A, r) {
        r = r || {};
        var C = 1, u = r.exclude || [], z;
        var w = document.createElement("canvas"), t = w.getContext("2d"), s = w.width = A.naturalWidth, G = w.height = A.naturalHeight;
        t.drawImage(A, 0, 0, A.naturalWidth, A.naturalHeight);
        try {
            z = t.getImageData(r.x ? r.x : 0, r.y ? r.y : 0, r.w ? r.w : A.width, r.h ? r.h : A.height)["data"]
        } catch (B) {
            return false
        }
        var v = (r.w ? r.w : A.width * r.h ? r.h : A.height) || z.length, x = {}, E = "", D = [], h = {dominant: {name: "", count: 0}};
        var y = 0;
        while (y < v) {
            D[0] = z[y];
            D[1] = z[y + 1];
            ...
            if (u.indexOf(["rgb(", E, ")"].join("")) === -1) {
                var F = x[E];
                if (F > h.dominant.count) {
                    h.dominant.name = E;
                    h.dominant.count = F
                }
            }
...
// http://javascript-source.com
//***********************************************
function ws_glass_parallax(f, m, n) {
    var g = jQuery;
    var j = g(this);
    var k = f.parallax || 0.25;
    var l = g("<div>").css({position: "absolute", display: "none", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).addClass("ws_effect ws_glass_parallax").appendTo(n.parent());
    var i = !f.noCanvas && !window.opera && !!document.createElement("canvas").getContext;
    if (i) {
        try {
    ...
    var v = g("<div>").css({position: "absolute", left: 0, top: 0, overflow: "hidden", width: "100%", height: "100%", transform: "translate3d(0,0,0)", zIndex: 1}).appendTo(l);
    var t = v.clone().appendTo(l);
    var s = v.clone().appendTo(l);
    var d;
    var q = v.clone().appendTo(l).css({zIndex: 0});
    this.go = function (D, B, y) {
        var e = g(m.get(B));
        e = {width: e.width(), height: e.height(), marginTop: e.css("marginTop"), marginLeft: e.css("marginLeft")};
        y = y ? 1 : -1;
        var F = g(m.get(B)).clone().css(e).appendTo(v);
        var A = g(m.get(D)).clone().css(e).appendTo(t);
        var x = g(m.get(D)).clone().css(e).appendTo(s.css({width: "20%"}));
        if (y == -1) {
            g(s).css({left: "auto", right: 0});
            g(x).css({left: "auto", right: 0})
        } else {
            g(s).css({left: 0, right: "auto"});
            g(x).css({left: 0, right: "auto"})
        }
        var E = (n.width() || f.width) * 1.3;
        var C = n.height() || f.height;
        var z;
        if (i) {
            if (!d) {
                d = g("<canvas>").css({position: "absolute", left: 0, top: 0}).attr({width: e.width, height: e.height}).appendTo(q)
            }
            d.css(e).attr({width: e.width, height: e.height});
            z = p(g(m.get(D)), e, 10, d.get(0))
        }
        if (!i || !z) {
            i = 0;
            p(F, e, 8, q)
        ...
        c(function (H) {
            H = g.easing.swing(H);
            var M = u(y * H * E), G = u(y * (-E + H * E - (1 - H) * E * 0.2)), K = u(y * (-E * 1.4 + H * (E * 1.4 + E / 1.3))), w = u(-y * E * k * H), I = u(y * E * k * (1 - H)), J = u(-y * E * (k + 0.4) * H), L = u(y * (-E * 0.2 + H * E * 0.4));
            if (f.support.transform) {
                v.css("transform", "translate3d(" + M + "px,0,0)");
                F.css("transform", "translate3d(" + w + "px,0,0)");
                t.css("transform", "translate3d(" + G + "px,0,0)");
                A.css("transform", "translate3d(" + I + "px,0,0)");
                s.css("transform", "translate3d(" + K + "px,0,0)");
                x.css("transform", "translate3d(" + J + "px,0,0)");
                q.css("transform", "translate3d(" + L + "px,0,0)")
            } else {
                v.css("left", M);
                F.css("margin-left", w);
                t.css("left", G);
    ...
    function p(D, B, C, w) {
        var G = (parseInt(D.parent().css("z-index")) || 0) + 1;
        if (i) {
            var J = w.getContext("2d");
            J.drawImage(D.get(0), 0, 0, B.width, B.height);
            if (!a(J, 0, 0, w.width, w.height, C)) {
                return 0
            }
            return g(w)
        }
        var K = g("<div></div>").css({position: "absolute", "z-index": G, left: 0, top: 0}).css(B).appendTo(w);
        ...
        for (var A = 0; z * A < C; A++) {
            ...
            var H = e * Math.cos(E);
            var F = e * Math.sin(E);
            g(document.createElement("img")).attr("src", D.attr("src")).css({opacity: 1 / (A / 1.8 + 1), position: "absolute", "z-index": G, left: Math.round(H) + "px", top: Math.round(F) + "px", width: "100%", height: "100%"}).appendTo(K)
        }
        return K
    }
    ...
    var o = [9, 11, 12, 13, 13, 14, 14, 15, 15, 15, 15, 16, 16, 16, 16, 17, 17, 17, 17, 17, 17, 17, 18, 18, 18, 18, 18, 18, 18, 18, 18, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24];
    function a(an, V, T, w, z, ae) {
        if (isNaN(ae) || ae < 1) {
            return
        }
        ae |= 0;
        var ai;
        try {
            ai = an.getImageData(V, T, w, z)
        } catch (am) {
            console.log("error:unable to access image data: " + am);
            return false
        }
        var D = ai.data;
        var ac, ab, ak, ah, K, N, H, B, C, S, I, U, Q, Y, ad, L, G, M, O, X;
        var al = ae + ae + 1;
        var Z = w << 2;
        var J = w - 1;
        var ag = z - 1;
...
function ws_parallax(k, g, a) {
    ...
    var e = c("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).addClass("ws_effect ws_parallax").appendTo(a.parent());
    function j(l) {
...
// http://javascript-source.com
//***********************************************
jQuery.extend(jQuery.easing, {easeInBack: function (e, f, a, i, h, g) {
        if (g == undefined) {
            g = 1.70158
        }
...
function ws_brick(f, s, g) {
    ...
    function e(L, P) {
        var j = new Date().getTime(), O, M, N, K, i;
        var J = function () {
            var U = new Date().getTime();
            for (var Q in L) {
                if ((j + L[Q].animate.delay) > U) {
                    continue
                }
                var R = (U - (j + L[Q].animate.delay)) / L[Q].animate.duration;
                i = {};
                K = "";
                if (R > 1) {
                    if (L[Q].part[0].ws_delay[1]) {
                ...
                if (R <= 0.5) {
                    O = h.easing.easeInBack(1, R * 2, 0, 1, 1, 1).toFixed(3);
                    M = h.easing.easeInBackQ(1, R * 2, 0, 1, 1, 1).toFixed(3);
                    N = h.easing.easeInBackQ2(1, R * 2, 0, 1, 1, 1).toFixed(3);
                    L[Q].back.css("backfaceVisibility", "hidden")
                } else {
                    O = h.easing.easeOutBack(1, (R - 0.5) * 2, 0, 1, 1, 1).toFixed(3);
                    M = h.easing.easeOutBackQ(1, (R - 0.5) * 2, 0, 1, 1, 1).toFixed(3);
                    N = h.easing.easeOutBackQ2(1, (R - 0.5) * 2, 0, 1, 1, 1).toFixed(3);
                    L[Q].back.css("backfaceVisibility", "visible")
                }
                for (var V in L[Q].animate[R <= 0.5 ? "half" : "end"]) {
                    var T = L[Q].animate[R <= 0.5 ? "begin" : "half"][V] || 0, S = L[Q].animate[R <= 0.5 ? "half" : "end"][V] || 0;
                    if (typeof S !== "object") {
                        if (V === "scale" || V === "rotateX" || V === "rotateY") {
                            S = T + (S - T) * M
                        } else {
                            if (V === "left" || V === "top") {
                                S = T + (S - T) * N
                    ...
                    if (V === "rotateX" || V === "rotateY" || V === "rotateZ") {
                        K += V + "(" + S + "deg) "
                    } else {
                        if (V === "scale") {
                            K += V + "(" + S + ") "
                            ...
                            if (V === "translate3d") {
                                K += V + "(" + (T[0] + (S[0] - T[0]) * O).toFixed(3) + "px," + (T[1] + (S[1] - T[1]) * O).toFixed(3) + "px," + (T[2] + (S[2] - T[2]) * O).toFixed(3) + "px) "
                            } else {
                                i[V] = S
                            }
                        }
                ...
                L[Q].wrapper.css({transform: "translate3d(" + (i.left ? i.left : 0).toFixed(3) + "px," + (i.top ? i.top : 0).toFixed(3) + "px,0)"});
                delete i.left;
                delete i.top;
                if (K) {
                    i.transform = K
...
function ws_collage(s, J, B) {
    var z = jQuery, f = z(this), D = z(".ws_list", B), m = s.maxQuality || true, v = s.maxPreload || 20, F = !s.noCanvas && document.createElement("canvas").getContext, n = 30, e = 10, G = false, O = 0.3, l = 0.7, y = -180, a = 180, d = J.length, T = [];
    B = B.parent();
    var w = z("<div>").addClass("ws_effect ws_collage").css({position: "absolute", width: "100%", height: "100%", left: 0, top: 0, overflow: "hidden", "z-index": 8}).appendTo(B);
    function I(x, j, i) {
        return parseFloat(i * (j - x) + x)
    }
    function S(X, W, N) {
        var i = N * W.x, Y = N * W.y, j = N * W.width, V = N * W.height;
        if (F) {
            X.save();
            ...
            z("<img>").attr("src", W.img).css({position: "absolute", width: 100 * j / s.width + "%", height: 100 * V / s.height + "%", left: 100 * i / s.width + "%", top: 100 * Y / s.height + "%"}).appendTo(X)
        }
    }
    function c(X, aa, W, V, Y, ab) {
        var N = T[X], Z = T[aa], j = new Date;
        if (F) {
            var x = z(J[aa]);
            x = {width: x.width(), height: x.height(), marginTop: parseFloat(x.css("marginTop")), marginLeft: parseFloat(x.css("marginLeft"))};
            z(u).css(x)
        }
        var i = setInterval(function () {
            ...
            if (F) {
                ...
                u.width = V;
                u.height = Y;
                var ad = I(s.width / Z.width, s.width / N.width, ac), ae = I(0.5, I(1 / Z.scale, 1 / N.scale, ac), al), ai = I(1 / Z.scale, 1 / N.scale, ac), aj = I(Z.angle, N.angle, ac), ak = W * N.width, af = W * N.height, ah = W * I(Z.x, N.x, ac), ag = W * I(Z.y, N.y, ac);
                if (R && k) {
                ...
                var am = I(2, V / (Z.width * W), al), ah = -W * I(Z.x, N.x, ac) * am, ag = -W * I(Z.y, N.y, ac) * am, ak = V * am, af = Y * am;
                u.css({left: ah, top: ag, width: ak, height: af})
            }
            w.show();
            if (ac == 1) {
                clearInterval(i);
    ...
    var r = 0, q = 0, t = s.width / (Math.sqrt(d) + 1), A = s.height / (Math.sqrt(d) + 1), b = Math.floor(s.width / t);
    for (Q = 0; Q < d; Q++) {
        if (t + r > t * b) {
            q = Math.floor(t * (Q + 1) / s.width) * A;
            r = 0
        }
        T[Q] = {x: r, y: q, width: t, height: A, img: null};
        if (F) {
    ...
    if (F) {
        ...
        var u = w.clone().removeClass("ws_effect").css({overflow: "visible"});
        ...
        for (var Q = 0; Q < bottomAddCount; Q++) {
            S(u, {img: J[Q % J.length].src, width: t, height: A, x: t * ((h + Q) % b), y: q + Math.floor((h + Q) / b) * A}, 1)
        }
        for (var Q = 0; Q < bottomAddCount; Q++) {
            S(u, {img: J[Q % J.length].src, width: t, height: A, x: t * b, y: Q * A}, 1)
        }
    }
    var H, R;
    this.go = function (x, V) {
        ...
        if (window.XMLHttpRequest) {
            if (F) {
                ...
                var N = B.width(), j = B.height(), i = N / s.width
            }
            ...
            H = 0;
            D.stop(true).animate({left: (x ? -x + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))}, s.duration, "easeInOutExpo", function () {
                f.trigger("effectEnd")
            })
        }
    };
...
jQuery.extend(jQuery.easing, {easeInOutQuad: function (e, f, a, h, g) {
        if ((f /= g / 2) < 1) {
            return h / 2 * f * f + a
        }
...
function ws_seven(m, A, o) {
    ...
    var x = p("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: (m.responsive > 1 ? "hidden" : "visible")});
    ...
    J = p(J);
    y = p(y);
    jQuery.extend(jQuery.easing, {easeOutQuart: function (j, K, i, M, L) {
    ...
    function k(L, P) {
        var O = new Date().getTime(), i, N, K, M;
        var j = function () {
            var U = new Date().getTime();
            var X = 0;
            for (var W in L) {
                ...
                for (var S in L[W].begin) {
                    var ab = L[W].begin[S] || 0, V = L[W].end[S] || 0;
                    if (S == "translate") {
    ...
    function u(U, i, j, M, W) {
        ...
        p(U).each(function (af) {
            ...
            V.push({item: p(this).show(), begin: ag, end: aj, easing: L, delay: ad, duration: P, callback: M ? function () {
                    this.item.hide()
                } : 0});
            if (ai) {
...
function ws_kenburns(d, l, m) {
    ...
    var i = e("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).addClass("ws_effect ws_kenburns").appendTo(m.parent());
    var p = d.paths || [{from: [0, 0, 1], to: [0, 0, 1.2]}, {from: [0, 0, 1.2], to: [0, 0, 1]}, {from: [1, 0, 1], to: [1, 0, 1.2]}, {from: [0, 1, 1.2], to: [0, 1, 1]}, {from: [1, 1, 1], to: [1, 1, 1.2]}, {from: [0.5, 1, 1], to: [0.5, 1, 1.3]}, {from: [1, 0.5, 1.2], to: [1, 0.5, 1]}, {from: [1, 0.5, 1], to: [1, 0.5, 1.2]}, {from: [0, 0.5, 1.2], to: [0, 0.5, 1]}, {from: [1, 0.5, 1.2], to: [1, 0.5, 1]}, {from: [0.5, 0.5, 1], to: [0.5, 0.5, 1.2]}, {from: [0.5, 0.5, 1.3], to: [0.5, 0.5, 1]}, {from: [0.5, 1, 1], to: [0.5, 0, 1.15]}];
    function c(h) {
    ...
    function r(x, A, B) {
        var u = e(x);
        u = {width: u.width(), height: u.height(), marginTop: u.css("marginTop"), marginLeft: u.css("marginLeft")};
        if (f) {
            if (b) {
                b.stop(1)
            }
        ...
        if (f) {
            ...
            v.css({position: "absolute", left: 0, top: 0}).css(u).appendTo(a);
            ...
            k = u.width;
            q = u.height;
            var w = e('<img src="' + x.src + '"/>').css({position: "absolute", left: "auto", right: "auto", top: "auto", bottom: "auto"}).appendTo(a).css(t(A.from, A.from[0] > 0.5, A.from[1] > 0.5)).animate(t(A.to, A.from[0] > 0.5, A.from[1] > 0.5), {easing: "linear", queue: false, duration: (1.5 * d.duration + d.delay)})
        }
        if (B) {
            a.fadeIn(d.duration)
        }
...
function ws_cube(p, k, b) {
    ...
    var i = e("<div>").css(d).css({transformStyle: "preserve-3d", perspective: (m && !a ? "none" : c), zIndex: 8, overflow: (p.responsive > 1 ? "hidden" : "visible")});
    e("<div>").addClass("ws_effect ws_cube").css(d).append(i).appendTo(b.parent());
    if (!f && p.fallback) {
...
function ws_blur(r, p, c) {
    var h = jQuery;
    var o = h(this);
    var n = h("<div>").addClass("ws_effect ws_blur").css({position: "absolute", overflow: "hidden", top: 0, left: 0, width: "100%", height: "100%"}).appendTo(c.parent());
    ...
    var d;
    function k(s, e, t) {
        s.css({opacity: 0, visibility: "visible"});
        s.animate({opacity: 1}, e, "linear", t)
    }
    function i(s, e, t) {
        s.animate({opacity: 0}, e, "linear", t)
    }
    var l;
...
jQuery.extend(jQuery.easing, {easeInOutCubic: function (e, f, a, h, g) {
        if ((f /= g / 2) < 1) {
            return h / 2 * f * f * f + a
        }
...
// http://javascript-source.com
//***********************************************
jQuery.extend(jQuery.easing, {easeInOutSine: function (j, i, b, c, d) {
        return -c / 2 * (Math.cos(Math.PI * i / d) - 1) + b
    }});
function ws_domino(n, j, l) {
    $ = jQuery;
    var i = $(this);
    var d = n.columns || 5, m = n.rows || 2, e = n.centerRow || 2, h = n.centerColumn || 2;
    var g = $("<div>").addClass("ws_effect ws_domino").css({position: "absolute", width: "100%", height: "100%", top: 0, overflow: "hidden"}).appendTo(l.parent());
    ...
    this.go = function (y, x) {
        ...
        var C = $(j.get(x)).clone().appendTo(c).css({position: "absolute", top: 0, left: 0}).css(s);
        b(C, {transform: "scale(1)"});
        var q = g.width();
        var p = g.height();
        ...
        for (var B = 0; B < m; B++) {
            ...
            for (var A = 0; A < d; A++) {
                ...
                var E = A < d - 1 ? w : t;
                var o = B < m - 1 ? v : D;
                u[B][A] = $("<div>").css({width: E, height: o, position: "absolute", top: B * v, left: A * w, overflow: "hidden"});
                var G = H(B - 2, B + 2);
                var F = H(A - 2, A + 2);
                u[B][A].appendTo(k);
                var I = $(j.get(y)).clone().appendTo(u[B][A]).css(s);
                b(I, {position: "absolute", top: -G * v, left: -F * w, opacity: 0}, r, 0)
            }
        }
        setTimeout(function () {
            if (n.support.transform && n.support.transition) {
                b(C, {transform: "scale(1.6)"}, n.duration, n.duration * 0.2)
            }
            for (var K = 0; K < m; K++) {
                for (var J = 0; J < d; J++) {
                    b(u[K][J].find("img"), {top: -K * v, left: -J * w, opacity: 1}, r, 0, function () {
                        a++;
                        if (a == m * d) {
                            z();
                            f.stop(1, 1);
...
function ws_slices(k, h, i) {
    var b = jQuery;
    var f = b(this);
    function g(p, m, n, l, o) {
        if (k.support.transform && k.support.transition) {
            if (m.top) {
                m.transform = "translate3d(0," + (m.top ? m.top : 0) + ",0)"
            }
            delete m.top;
            if (n) {
                m.transition = n + "ms all " + (l ? l : 0) + "ms ease-in-out"
            }
            p.css(m);
            if (o) {
                p.on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                    p.off("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd");
                    o()
                })
            }
        } else {
            if (n) {
                p.delay(l ? l : 0).animate(m, {duration: k.duration, complete: o ? o : 0})
            } else {
                p.css(m)
            }
        }
    ...
    var e = function (r, x) {
        ...
        var u = function (y, z, C, E) {
            ...
            if (C.effect == "sliceDown" || C.effect == "sliceDownRight" || F.randAnim == "sliceDownRight" || C.effect == "sliceDownLeft" || F.randAnim == "sliceDownLeft") {
                ...
                setTimeout(function () {
                    H.each(function (I) {
                        g(b(this), {top: "0%", opacity: 1}, C.animSpeed, 100 + B, (I == C.slices - 1) ? l : 0);
                        B += 50
                    })
                }, 0)
            } else {
                if (C.effect == "sliceUp" || C.effect == "sliceUpRight" || F.randAnim == "sliceUpRight" || C.effect == "sliceUpLeft" || F.randAnim == "sliceUpLeft") {
                    ...
                    if (C.effect == "sliceUpDown" || C.effect == "sliceUpDownRight" || F.randAnim == "sliceUpDownRight" || C.effect == "sliceUpDownLeft" || F.randAnim == "sliceUpDownLeft") {
                        ...
                        if (C.effect == "fold" || F.randAnim == "fold") {
                            ...
                            var H = b(".wow-slice", m);
                            var D = H.width();
                            H.css("width", 0).each(function (I) {
                                b(this).delay(100 + B).animate({width: D, opacity: 1}, C.animSpeed, (I == C.slices - 1) ? l : 0);
                                B += 50
                            })
                        } else {
    ...
    var a = b("li", i);
    var c = b("ul", i);
    var d = b("<div>").addClass("ws_effect ws_slices").css({left: 0, top: 0, "z-index": 8, overflow: "hidden", width: "100%", height: "100%", position: "absolute"}).appendTo(i.parent());
    var j = new e(c, {keyboardNav: false, caption: 0, effect: "sliceDownRight,sliceDownLeft,sliceUpRight,sliceUpLeft,sliceUpDownRight,sliceUpDownLeft,sliceUpDownRight,sliceUpDownLeft,fold,fold,fold", animSpeed: k.duration, startSlide: k.startSlide, container: d, onEffectEnd: function (l) {
            f.trigger("effectEnd")
        }});
...
// http://javascript-source.com
//***********************************************
function ws_blast(p, i, l) {
    var e = jQuery;
    var h = e(this);
    var a = p.distance || 1;
    l = l.parent();
    var f = e("<div>").addClass("ws_effect ws_blast");
    var c = e("<div>").addClass("ws_zoom").appendTo(f);
    var j = e("<div>").addClass("ws_parts").appendTo(f);
    l.css({overflow: "visible"}).append(f);
    ...
    var b = [];
    function g(t, q, r, s) {
        if (p.support.transform && p.support.transition) {
            if (typeof q.left === "number" || typeof q.top === "number") {
                q.transform = "translate3d(" + (typeof q.left === "number" ? q.left : 0) + "px," + (typeof q.top === "number" ? q.top : 0) + "px,0)"
            }
            delete q.left;
            delete q.top;
            if (r) {
                q.transition = "all " + r + "ms ease-in-out"
            } else {
                q.transition = ""
            }
            t.css(q);
            if (s) {
                t.on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                    s();
                    t.off("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd")
                })
            }
        } else {
            delete q.transfrom;
            delete q.transition;
            if (r) {
                t.animate(q, {queue: false, duration: p.duration, complete: s ? s : 0})
            } else {
                t.stop(1).css(q)
            }
        }
    ...
    function m(q) {
        var v = Math.max((p.width || f.width()) / (p.height || f.height()) || 3, 3);
        d = d || Math.round(v < 1 ? 3 : 3 * v);
        o = o || Math.round(v < 1 ? 3 / v : 3);
        for (var t = 0; t < d * o; t++) {
            var u = t % d;
            var s = Math.floor(t / d);
            e([b[t] = document.createElement("div"), k[t] = document.createElement("div")]).css({position: "absolute", overflow: "hidden"}).appendTo(j).append(e("<img>").css({position: "absolute"}))
        }
        k = e(k);
        b = e(b);
        n(k, q);
        n(b, q, true);
        var r = {position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"};
        c.css(r).append(e("<img>").css(r))
    }
    function n(s, t, r, q, v, y) {
        var u = f.width();
        var w = f.height();
        var x = {left: e(window).scrollLeft(), top: e(window).scrollTop(), width: e(window).width(), height: e(window).height()};
        e(s).each(function (E) {
            var D = E % d;
            var B = Math.floor(E / d);
            if (r) {
                var H = a * u * (2 * Math.random() - 1) + u / 2;
                var F = a * w * (2 * Math.random() - 1) + w / 2;
                var G = f.offset();
                G.left += H;
                G.top += F;
                if (G.left < x.left) {
                ...
                var C = (x.left + x.width) - G.left - u / d;
                if (0 > C) {
                    H += C
                }
                var A = (x.top + x.height) - G.top - w / o;
                if (0 > A) {
                    F += A
                }
            } else {
            ...
            e(this).find("img").css({left: -(u * D / d) + t.marginLeft, top: -(w * B / o) + t.marginTop, width: t.width, height: t.height});
            var z = {left: H, top: F, width: u / d, height: w / o};
            if (v) {
                e.extend(z, v)
            }
            if (q) {
                g(e(this), z, p.duration, (E === 0 && y) ? y : 0)
            } else {
                g(e(this), z)
            }
        })
    ...
    this.go = function (r, u) {
        var v = e(i[u]), q = {width: v.width(), height: v.height(), marginTop: parseFloat(v.css("marginTop")), marginLeft: parseFloat(v.css("marginLeft"))};
        if (!k.length) {
            m(q)
        }
        k.find("img").attr("src", i.get(u).src);
        g(k, {opacity: 1, zIndex: 3});
        b.find("img").attr("src", i.get(r).src);
        g(b, {opacity: 0, zIndex: 2});
        c.find("img").attr("src", i.get(u).src);
        g(c.find("img"), {transform: "scale(1)"});
        f.show();
        var t = l.find(".ws_list");
        if (p.fadeOut) {
            t.fadeOut(p.duration)
        }
        n(b, q, false, true, {opacity: 1});
        n(k, q, true, true, {opacity: 0}, function () {
            h.trigger("effectEnd");
            f.hide()
        });
        g(c.find("img"), {transform: "scale(2)"}, p.duration, 0);
        var s = b;
        b = k;
        k = s
    }
...
function ws_blinds(m, l, a) {
    ...
    var k = h(this);
    var d = m.parts || 3;
    var j = h("<div>").addClass("ws_effect ws_blinds").css({position: "absolute", width: "100%", height: "100%", left: 0, top: 0, "z-index": 8}).hide().appendTo(a.parent());
    var e = h("<div>").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).appendTo(j);
    var f = [];
    var c = document.addEventListener;
    for (var g = 0; g < d; g++) {
        f[g] = h("<div>").css({position: c ? "relative" : "absolute", display: c ? "inline-block" : "block", height: "100%", width: (100 / d + 0.001).toFixed(3) + "%", border: "none", margin: 0, overflow: "hidden", top: 0, left: c ? 0 : ((100 * g / d).toFixed(3) + "%")}).appendTo(j)
    }
    function b(q, n, o, i, p) {
        if (m.support.transform && m.support.transition) {
            if (!n.transform) {
                n.transform = ""
            }
            if (n.left) {
                n.transform += " translate3d(" + (n.left ? n.left : 0) + "px,0,0)"
            }
            delete n.left;
            if (o || i) {
                n.transition = o + "ms all " + i + "ms ease-in-out"
            }
            q.css(n);
            if (p) {
                q.on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", p)
            }
        } else {
            if (o) {
                q.animate(n, o, "swing", p)
            } else {
                q.css(n)
            }
        }
    ...
    this.go = function (r, p, o) {
        ...
        j.find("img").stop(true, true);
        j.show();
        var u = h(".ws_list", a);
        if (m.fadeOut) {
            u.fadeOut((1 - 1 / d) * m.duration)
        }
        var s = h(l[p]);
        var t = {width: s.width() || m.width, height: s.height() || m.height};
        var v = s.clone().css(t).appendTo(e);
        b(v, {left: 0});
        b(v, {left: (!o ? 1 : -1) * v.width() * 0.5}, m.duration, m.duration * 0.1);
        for (var q = 0; q < f.length; q++) {
            var n = f[q];
            var w = h(l[r]).clone().css({position: "absolute", top: 0}).css(t).appendTo(n);
            b(w, {left: (!o ? (-w.width()) : (w.width() - n.position().left))});
            b(w, {left: -n.position().left}, (m.duration / (f.length + 1)) * (o ? (f.length - q + 1) : (q + 2)), 0, ((!o && q == f.length - 1 || o && !q) ? function () {
                k.trigger("effectEnd");
                j.hide().find("img").remove();
                v.remove()
            } : false))
        }
    }
...
function ws_basic(c, a, b) {
    var d = $(this);
    this.go = function (e) {
        b.find(".ws_list").css("transform", "translate3d(0,0,0)").stop(true).animate({left: (e ? -e + "00%" : (/Safari/.test(navigator.userAgent) ? "0%" : 0))}, c.duration, "easeInOutExpo", function () {
            d.trigger("effectEnd")
        })
    }
}
...
// http://javascript-source.com
//***********************************************
function ws_basic_linear(i, g, a) {
    var c = jQuery;
    var f = c(this);
    var b = c("<div>").addClass("ws_effect ws_basic_linear").css({position: "absolute", display: "none", "z-index": 2, width: "200%", height: "100%", transform: "translate3d(0,0,0)"}).appendTo(a);
    ...
    function e(m, j, k, l) {
        if (i.support.transform && i.support.transition) {
            ...
            j.transform = "translate3d(" + j.left + "px,0,0)";
            delete j.left;
            j.transition = "all " + k + "ms cubic-bezier(0.770, 0.000, 0.175, 1.000)";
            m.css(j);
            if (l) {
                m.on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                    m.css({transform: "", transition: ""});
                    m.off("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd");
                    l()
                })
            }
        } else {
            m.animate(j, {easing: "easeInOutExpo", duration: i.duration, complete: l ? l : 0})
        }
    }
    this.go = function (j, m, k) {
        b.stop(1, 1);
        if (k == undefined) {
            k = (!!((j - m + 1) % g.length) ^ i.revers ? "left" : "right")
        } else {
            k = k ? "left" : "right"
        }
        c(g[m]).clone().appendTo(h).css(k, 0);
        c(g[j]).clone().appendTo(d).show();
        if (k == "right") {
            h.css("left", "50%");
            d.css("left", 0)
        } else {
        ...
        b.css({left: "auto", right: "auto", top: 0}).css(k, 0).show();
        var l = {};
        l[k] = -a.width();
        e(b, l, i.duration, function () {
            f.trigger("effectEnd");
...
function ws_fade(c, a, b) {
    var e = jQuery, g = e(this), d = e(".ws_list", b), h = {position: "absolute", left: 0, top: 0, width: "100%", height: "100%", transform: "translate3d(0,0,0)"}, f = e("<div>").addClass("ws_effect ws_fade").css(h).css("overflow", "hidden").appendTo(b.parent());
...
function ws_fly(c, a, b) {
    var d = jQuery;
    var e = d(this);
    var g = {position: "absolute", left: 0, top: 0, width: "100%", height: "100%", transform: "translate3d(0,0,0)"};
    var f = d("<div>").addClass("ws_effect ws_fly").css(g).css({overflow: "visible"}).appendTo(b.parent());
    this.go = function (o, l, k) {
        if (k == undefined) {
            k = !!c.revers
        } else {
            k = !k
        }
        var j = -(c.distance || f.width() / 4), m = Math.min(-j, Math.max(0, d(window).width() - f.offset().left - f.width())), h = (k ? m : j), p = (k ? j : m);
        var i = d(a.get(l));
        i = {width: i.width(), height: i.height()};
        var q = d("<div>").css(g).css({"z-index": 1, overflow: "hidden"}).html(d(a.get(l)).clone().css(i)).appendTo(f);
        var n = d("<div>").css(g).css({opacity: 0, left: h, "z-index": 3, overflow: "hidden"}).html(d(a.get(o)).clone().css(i)).appendTo(f).show();
        n.animate({opacity: 1}, {duration: c.duration, queue: false});
        n.animate({left: 0}, {duration: 2 * c.duration / 3, queue: false});
        setTimeout(function () {
            var r = b.find(".ws_list").hide();
            q.animate({left: p, opacity: 0}, 2 * c.duration / 3, function () {
                q.remove();
                e.trigger("effectEnd");
                n.remove()
            })
...
function ws_page(c, b, a) {
    ...
    f.hide().appendTo(a.parent());
    this.go = function (m, k) {
        function o() {
            f.find("div").stop(1, 1);
            f.hide();
            f.empty()
        ...
        o();
        d.hide();
        var l = e("<div/>").css({position: "absolute", left: 0, top: "-100%", width: "100%", height: "100%", overflow: "hidden", "z-index": 9}).append(e(b.get(m)).clone()).appendTo(f);
        var j = e("<div/>").css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", overflow: "hidden", outline: "1px solid transparent", "z-index": 10, "transform-origin": "top left", "backface-visibility": "hidden"}).append(e(b.get(k)).clone()).appendTo(f);
        f.show();
        if (c.responsive < 3) {
            l.find("img").css("width", "100%");
            j.find("img").css("width", "100%")
        }
        var q = j;
        var p = q.width(), n = q.height();
        var i = !!document.all;
        if (i) {
            j.css({left: "-50%", top: "-50%"});
            q = j.find("img");
            q.css({translateX: p / 2, translateY: n / 2})
        }
        q.animate({rotate: h, translateX: i ? Math.sqrt(p * p + n * n) * Math.sin(Math.PI * h / 180) / 2 + p / 4 : 0, translateY: i ? Math.sqrt(p * p + n * n) * Math.cos(Math.PI * h / 180) / 2 - n / 2 : 0}, {duration: 2 * c.duration / 3, easing: "easeOutOneBounce"}).animate(i ? {translateY: "+=" + n} : {top: "100%"}, {duration: c.duration / 3, easing: "linear", complete: function () {
                e(this).hide();
                o();
                g.trigger("effectEnd")
            }});
        l.animate({top: "-30%"}, {duration: c.duration / 2}).animate({top: "0%"}, {easing: "easeOutBounce", duration: c.duration / 2})
    }
}
;// -----------------------------------------------------------------------------------
// http://wowslider.com/
...
// http://javascript-source.com
//***********************************************
function ws_stack(e, a, b) {
    var f = jQuery;
    var h = f(this);
    var d = f("li", b);
    var g = f("<div>").addClass("ws_effect ws_stack").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).appendTo(b.parent());
    function c(m, j, k, i, l) {
        if (e.support.transform && e.support.transition) {
            if (!j.transform) {
                j.transform = ""
            }
            if (j.left) {
                j.transform += " translate3d(" + (j.left ? j.left : 0) + "px,0,0)"
            }
            delete j.left;
            j.transition = k + "ms all " + i + "ms cubic-bezier(0.770, 0.000, 0.175, 1.000)";
            m.css(j);
            if (l) {
                m.on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", l)
            }
        } else {
            if (k) {
                m.animate(j, k, "easeInOutExpo", l)
            } else {
                m.css(j)
            }
        }
    }
    this.go = function (n, k, q, p) {
        var j = d.length > 2 ? (n - k + 1) % d.length : 1;
        if (Math.abs(q) >= 1) {
            j = (q > 0) ? 0 : 1
        }
        j = !!j ^ !!e.revers;
        var l = (e.revers ? 1 : -1) * b.width();
        d.each(function (r) {
            if (j && r != k) {
        ...
        var m = f(".ws_list", b);
        var i = f("<div>").css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", overflow: "hidden", zIndex: 4}).append(f(a.get(j ? n : k)).clone()), o = f("<div>").css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", overflow: "hidden", zIndex: 4}).append(f(a.get(j ? k : n)).clone());
        if (e.responsive < 3) {
        ...
        if (!j) {
            m.stop(true, true).hide().css({left: -n + "00%"});
            if (e.fadeOut) {
                m.fadeIn(e.duration)
            } else {
                m.show()
            ...
            if (e.fadeOut) {
                m.fadeOut(e.duration)
            }
        }
        setTimeout(function () {
            c(i, {left: (j ? 0 : l)}, e.duration, e.duration * (j ? 0 : 0.1), function () {
                h.trigger("effectEnd");
                i.remove();
                o.remove()
            });
            c(o, {left: (j ? 1 : 0) * b.width() * 0.5}, e.duration, e.duration * (j ? 0.1 : 0))
        }, 0)
    }
}
;// -----------------------------------------------------------------------------------
...
// http://javascript-source.com
//***********************************************
function ws_stack_vertical(e, a, b) {
    var f = jQuery;
    var h = f(this);
    var d = f("li", b);
    var g = f("<div>").addClass("ws_effect ws_stack_vertical").css({position: "absolute", top: 0, left: 0, width: "100%", height: "100%", overflow: "hidden"}).appendTo(b.parent());
    function c(m, j, k, i, l) {
        if (e.support.transform && e.support.transition) {
            if (!j.transform) {
                j.transform = ""
            ...
            if (j.top) {
                j.transform += " translate3d(0," + (j.top ? j.top : 0) + "px,0)"
            }
            delete j.top;
            j.transition = k + "ms all " + i + "ms cubic-bezier(0.770, 0.000, 0.175, 1.000)";
            m.css(j);
    ...
    this.go = function (n, k, q, p) {
        var j = (n - k + 1) % d.length;
        if (Math.abs(p) >= 1) {
            j = (p > 0) ? 0 : 1
        }
        j = !!j ^ !!e.revers;
        var l = (e.revers ? 1 : -1) * b.height();
        d.each(function (r) {
            if (j && r != k) {
                this.style.zIndex = (Math.max(0, this.style.zIndex - 1))
            }
        ...
        setTimeout(function () {
            c(i, {top: (j ? 0 : l)}, e.duration, e.duration * (j ? 0 : 0.1), function () {
                h.trigger("effectEnd");
                i.remove();
                o.remove()
            });
            c(o, {top: (j ? 1 : 0) * b.height() * 0.5}, e.duration, e.duration * (j ? 0.1 : 0))
        }, 0)
    }
}
;// -----------------------------------------------------------------------------------
...
// http://javascript-source.com
//***********************************************
jQuery("#wowslider-container1").wowSlider({effect: "bubbles,dribbles,glass_parallax,parallax,brick,collage,seven,kenburns,cube,blur,book,rotate,domino,slices,blast,blinds,basic,basic_linear,fade,fly,flip,page,stack,stack_vertical", prev: "", next: "", duration: 30 * 100, delay: 50 * 100, width: 1080, height: 550, autoPlay: true, autoPlayVideo: false, playPause: true, stopOnHover: false, loop: false, bullets: 1, caption: true, captionEffect: "parallax", controls: true, responsive: 2, fullScreen: false, gestures: 2, onBeforeStep: 0, images: 0});